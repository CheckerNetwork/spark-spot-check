...
import { MAX_BUFER_SIZE } from './constants.js'
...
export default class SpotChecker {
  ...
  /**
       ...
       * */
  async executeSpotCheck ({ task, stats, entityBytesRange }) {
  ...
  /**
       ...
       * @param {EntityBytesRange} args.entityBytesRange
       * */
  async fetchCAR ({ protocol, address, cid, stats, entityBytesRange }) {
    ...
    const carBuffer = new ArrayBuffer(0, { maxByteLength: MAX_BUFER_SIZE })
    const carBytes = new Uint8Array(carBuffer)

    try {
      const url = getRetrievalUrl({ protocol, address, cid, entityBytesRange })
  ...
  /**
       ...
       * */
  async spotCheck (task, entityBytesRange) {
    const stats = newStats()
    await this.executeSpotCheck({ task, stats, entityBytesRange })
  ...
  /**
       ...
       * @param {EntityBytesRange} args.entityBytesRange
       * */
  async run ({ roundId, maxTasks, retrievalTasks, minerId, entityBytesRange }) {
    const results = []
    ...
    for (const task of retrievalTasks) {
      try {
        const result = await this.spotCheck(task, entityBytesRange)
 ...
 * @returns {string}
 * */
export function getRetrievalUrl ({ protocol, address, cid, entityBytesRange }) {
  if (protocol === 'http') {
    const baseUrl = multiaddrToHttpUrl(address)
    return `${baseUrl}/ipfs/${cid}?dag-scope=entity&entity-bytes=${entityBytesRange}`
  }
...
async function verifyContent (cid, carBytes) {
  ...
  for await (const root of roots) {
    if (root.toString() !== cid.toString()) {
      throw Object.assign(
        new Error(`Unexpected block CID ${root.cid}. Expected: ${cid}`),
