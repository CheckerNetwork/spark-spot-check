...
export default class SpotChecker {
    ...
    async executeSpotCheck(retrieval, stats, dagScope) {
        ...
        await this.fetchCAR(provider.protocol, provider.address, retrieval.cid, stats, dagScope)
    }

    async fetchCAR(protocol, address, cid, stats, dagScope) {
        ...
        const carBuffer = new ArrayBuffer(0)
    ...
    async spotCheck(retrieval, dagScope) {
        const stats = newStats()
        await this.executeSpotCheck(retrieval, stats, dagScope)

        Zinnia.jobCompleted()
        return stats
    }

    async run({ roundNum, numTasks, dagScope = 'all' }) {
        const results = []
        const retrievalTasks = await this.#tasker.getRetrievalTasks({ roundNum, numTasks })
...
/**
* 
* @param {'bitswap' | 'graphsync' | 'http'} protocol
* @param {string} address
* @param {string} cid
* @param {'all' | 'block' | 'entity'} dagScope
*/
export function getRetrievalUrl(protocol, address, cid, dagScope) {
    if (protocol === 'http') {
        const baseUrl = multiaddrToHttpUrl(address)
        return `${baseUrl}/ipfs/${cid}?dag-scope=${dagScope}`
    }
