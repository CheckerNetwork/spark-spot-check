...
export default class SpotChecker {
  ...
  /**
          ...
          * @param {EntityBytesRange} [args.entityBytesRange]
      */
  async run ({ roundId, maxTasks, minerId, dagScope = 'entity', entityBytesRange }) {
    const results = []
    const retrievalTasks = await this.#tasker.getRetrievalTasks({ roundId, maxTasks, minerId, entityBytesRange })
